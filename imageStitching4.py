...
def main():
    ...
    rainier1 = cv.imread("Train.jpg")
    rainier2 = cv.imread("Query.jpg")

    # Create output folder
    create_directory("ImageStitching/output")
...
def stitch_rainier(rainier1, rainier2, ransacIterations, ransacThreshold):
    ...
    stitched_image = stitch_two_images(rainier1, rainier2,
                                       siftDisplayName="2. SIFT matches for Rainier 1-2 (ImageStitching/output/2.png)",
                                       siftFilePath="project_images/output/2.png",
                                       saveSiftMatches=True,
                                       ransacDisplayName="3. RANSAC Matches for Rainier1 and Rainier2 (ImageStitching/output/3.png)",
                                       saveRansacMatches=True,
                                       ransacFilePath="project_images/output/3.png",
                                       ransacIterations=ransacIterations,
                                       ransacThreshold=ransacThreshold
                                       )
    Rainier1_2_file_path = "ImageStitching/output/4.png"
    ...
    cv.imshow("4. Stitched Rainier1 and Rainier2 (R1-2) (ImageStitching/output/4.png)", stitched_image)
    cv.waitKey(0)
    cv.destroyAllWindows()

    # Stitching Rainier3, Rainier4, Rainier5, Rainier6
    for i in range(3, 7):  # We want to load Rainier 3-6
        image = cv.imread("ImageStitching/Rainier" + str(i) + ".png")
    ...
    stitched_file_path = "ImageStitching/output/5.png"
    ...
    cv.imshow("9. Stitched Rainier1-6 (Scaled 50% for Displaying) (ImageStitching/output5.png)", stitched_image)
    cv.waitKey(0)
    cv.destroyAllWindows()
